...
to setup
  ...
  [
    sprout 1
      [ set breed cars set color white set shape "car" set speed .8 set size 1.5
  ...
  [
    sprout 1
      [ set breed bicycles set speed .3 set size 1.5
  set speed-limit max_speed_bikes set speed-min .05 set energy random 100 set VRUdensity 0 set color black set shape "person" set heading one-of [ 0 90 180 270 ] set crashed 0 ]
  ]
...
to collide ;count collisions - collision risk reduces at rate proportional to newassociation strength
  if speed > 0 and any? bicycles-on patch-here and (newassociationstrength * 10 ) < random 10  [ set collisionsbikes 1 set shape "star" ]
...
to
  go
   if ticks >= 2000 [ stop ]
    ...
    if speed > speed-limit  [ set speed speed-limit ]
    fd speed ]
        ask cars [ separate-cars max-turtles-cars collide death turntoo calculatecarefactor remember resetinitial colour tracker ]
        ask bicycles [ bike-energy iceblock death turn morebikes hadacrash ]
    check-bicycles
...
to remember ;; if cars see a bike ahead of them, they remember that they have seen a bike - This is the gateway to losing memory that there were bikes on the road you just travelled on
